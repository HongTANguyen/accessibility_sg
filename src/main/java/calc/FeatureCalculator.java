...
public class FeatureCalculator {
    ...
    public static void calculate(Network routingNetwork, SimpleFeatureCollection collection,
        ...
        if (nodeResults != null) {
            nodesPerZone = AccessibilityUtils.assignNodesToZones(features,nodeResults.keySet(),routingNetwork);
        }

        // Precalculate marginal travel times
        Map<Id<Link>,Double> marginalTravelTimes = precalculateLinkMarginalDisutilities(routingNetwork, new OnlyTimeDependentTravelDisutility(travelTime), 0.,PERSON, vehicle);

        // Precalculate marginal disutilities
        Map<Id<Link>,Double> marginalDisutilities = precalculateLinkMarginalDisutilities(routingNetwork, travelDisutility, 0.,PERSON, vehicle);
    ...
    private static Map<Id<Link>,Double> precalculateLinkMarginalDisutilities(Network network, TravelDisutility disutility, double time, Person person, Vehicle vehicle) {
        ...
        for(Link link : network.getLinks().values()) {
            ...
            if(disutility instanceof CompositeDisutility) {
                linkDisutility -= ((CompositeDisutility) disutility).getJunctionComponent(link,vehicle);
            }
